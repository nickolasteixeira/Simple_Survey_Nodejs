...
function printSurvey (data) {
  return new Promise((resolve, reject) => {
    ...
    if (data.survey[0].answer.length !== 0) { console.log('-- Survey Questions and Answers --') } else { console.log('-- Survey Questions --') }
...
function formatSurvey (data) {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].survey[0].answer.length !== 0) { printSurvey(data[i]) }
  }
...
function askForSurvey () {
  return new Promise((resolve, reject) => {
    console.log('-------------------- ** ----------------------------')
    let surveyId = readlineSync.question('Please enter the Survey ID number: ')
    console.log('-------------------- ** ----------------------------')
...
function transformSurvey (post) {
  return new Promise((resolve, reject) => {
    ...
    /* Check for valid id */
    if (survey === undefined) { reject(post) }
    /* loop through survey and answer questsion */
    for (let i = 0; i < survey.length; i++) {
      let answer = readlineSync.question(`${survey[i].question} (y/n): `).toLowerCase()
      answer = (answer === 'y')
...
function displayUpdatePrompt () {
  ...
  let intId = parseInt(surveyId, 10)
  if (isNaN(intId)) {
    console.log(`${surveyId} is not a valid integer`)
    return
  }
...
function askQuestions (post) {
  return new Promise((resolve, reject) => {
    ...
    if (allQuestions.length < 1) { reject('No questions were added') }
